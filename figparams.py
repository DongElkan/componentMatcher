...
def set_figureparams(curves,rt,axes,texts):
    ...
    if figname is 'raw':
        ...
        x1,y1 	=0.8,0.93
        bbox 	={'fc': '#3CB371', 'ec': '#FF6347', 'boxstyle': 'round,pad=.3'}
        ...
        # text parameters for selecting wavelength
        x1,y1 	=0.84,0.93
        bbox 	={'fc': '#FFF8DC', 'ec': '#FF6347', 'boxstyle': 'round,pad=.2'}
        # text position for showing data information
...
def set_textcoords(xaxis,ymaxaxis,yminaxis,axislim,textxys,xycoords,
    ...
    rightxpercent=np.arange(xcp,min(1.-textlen,xcp+6./lx),0.01)
    leftxpercent =np.arange(max(textlen,xcp-6./lx),xcp-0.02,0.01)
    ...
    if ypp >= 0.1:
        ...
        if len(xl) is not 0 and len(xr) is 0:
            return xl[0],yl[0],-0.2, None, None
    ...
    # is adopted
    x,y,r=rightxpercent[0]*lx+xlimil,.9*ly+yliml,0.2
    return x, y, r, None, None
